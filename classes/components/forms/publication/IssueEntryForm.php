...
class IssueEntryForm extends FormComponent
{
    ...
    protected function addStampedIdentityField(Publication $publication, Server $server): void
    {
        ...
        if ($publication->getData('contextAbbreviation') && ($abbreviation = $publication->getPrimaryContextAbbreviation($server))) {
            $parts[] = '<li><strong>' . htmlspecialchars(__('manager.setup.serverAbbreviation')) . ':</strong> '
