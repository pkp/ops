...
b=b.find('textarea[name="personalMessage"]');b=tinyMCE.EditorManager.get(b.attr("id"));!1!==c&&""!==c.content&&b.setContent(c.content);return c.status}})(jQuery);(function(a){a.pkp.controllers.grid.users.reviewer=a.pkp.controllers.grid.users.reviewer||{};a.pkp.controllers.grid.users.reviewer.form=a.pkp.controllers.grid.users.reviewer.form||{};a.pkp.controllers.grid.users.reviewer.form.ReviewReminderFormHandler=function(b,c){this.parent(b,c);c.templateUrl&&(this.templateUrl_=c.templateUrl);b.find("#template").change(this.callbackWrapper(this.selectTemplateHandler_))};a.pkp.classes.Helper.inherits(a.pkp.controllers.grid.users.reviewer.form.ReviewReminderFormHandler,
a.pkp.controllers.form.AjaxFormHandler);a.pkp.controllers.grid.users.reviewer.form.ReviewReminderFormHandler.prototype.templateUrl_=null;a.pkp.controllers.grid.users.reviewer.form.ReviewReminderFormHandler.prototype.selectTemplateHandler_=function(){var b=this.getHtmlElement();a.post(this.templateUrl_,b.find("#template").serialize(),this.callbackWrapper(this.updateTemplate_),"json")};a.pkp.controllers.grid.users.reviewer.form.ReviewReminderFormHandler.prototype.updateTemplate_=function(b,c){var d=
this.getHtmlElement();b=this.handleJson(c);if(!b.status)return!1;c=c.content||{};var e=c.body||"";d=d.find('textarea[name="message"]');var f=tinyMCE.EditorManager.get(d.attr("id"));c.variables&&d.attr("data-variables",JSON.stringify(c.variables));f?f.setContent(e):d.val(e);return b.status}})(jQuery);(function(a){a.pkp.controllers.grid.users.stageParticipant=a.pkp.controllers.grid.users.stageParticipant||{};a.pkp.controllers.grid.users.stageParticipant.StageParticipantGridHandler=function(b,c){this.parent(b,c);this.bind("dataChanged",function(){this.refreshGridHandler();a("#submissionEditorDecisionsDiv,#copyeditingEditorDecisionsDiv,[id^=reviewDecisionsDiv]").each(function(){a.pkp.classes.Handler.getHandler(a(this)).reload()})})};a.pkp.classes.Helper.inherits(a.pkp.controllers.grid.users.stageParticipant.StageParticipantGridHandler,
