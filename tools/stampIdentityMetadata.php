 ...
 * @ingroup tools
 *
 * @brief CLI tool to re-stamp server identity metadata onto publications from the server's
...
class StampServerIdentityMetadata extends CommandLineTool
{
    // =========================================================================
    // OVERRIDE SECTION
    // Fill in these values if the data to be stamped differs from the server's
    // current live settings. Leave as null to use the server's current settings.
    // =========================================================================

    /**
     * Server name per locale.
     * e.g. ['en' => 'Old Server Name', 'fr' => 'Ancien nom de serveur']
     *
    ...
    /**
     * Server abbreviation per locale, as used in citations. Empty values are ignored.
     * e.g. ['en' => 'Old Abbrev.']
     *
     * @var array<string,string>|null
     */
    public ?array $contextAbbreviationOverride = null;
    ...
    {
        echo "Re-stamps server identity metadata (name, abbreviation, publisher location) onto publications\n"
            . "from the server's current settings. Only posted publications are stamped;\n"
            . "unpublished ones are stamped when they are posted.\n"
            ...
            . "Use 'all' to stamp everything: all posted publications in the server.\n";
    }
    ...
    /**
     * Stamp all posted publications in the server.
     */
    protected function stampAll(Server $context): void
    {
        $count = 0;
    ...
    /**
     * Stamp all posted publications of a single submission.
     */
    protected function stampSubmission(int $submissionId, Server $context): void
    {
    ...
    /**
     * Stamp publications posted in the given years.
     */
    protected function stampByYear(Server $context, array $years): void
    {
        $count = 0;
        $publications = Repo::publication()->getCollector()
            ->filterByContextIds([$this->contextId])
        ...
        if ($count > 0) {
            printf("Stamped %d publication(s) posted in the given year(s).\n", $count);
        } else {
            echo "No publications found posted in the given year(s).\n";
        }
    }

    /**
     * Stamp all posted publications of a submission, returning the count stamped.
     */
    protected function stampPublications(int $submissionId, Server $context): int
    {
        $count = 0;
        $publications = Repo::publication()->getCollector()
    ...
    protected function applyOverrides(object $object): void
    {
        ...
        if ($this->contextAbbreviationOverride !== null) {
            foreach (array_filter($this->contextAbbreviationOverride) as $locale => $abbreviation) {
                $object->setData('contextAbbreviation', $abbreviation, $locale);
            }
        }
