...
<article class="obj_preprint_details">
	...
	<span class="preprint_version">{translate key="submission.versionIdentity" datePublished=$publication->getData('datePublished')|date_format:$dateFormatShort version=$publication->getData('versionString')}</span>
					...
					<div class="value">
						{if count($parsedCitations)}
							{foreach from=$parsedCitations item="parsedCitation"}
								<p>{$parsedCitation->getRawCitationWithLinks()|strip_unsafe_html} {call_hook name="Templates::Preprint::Details::Reference" citation=$parsedCitation}</p>
							{/foreach}
						{else}
