...
<div class="page page_category">
	...
	<div class="about_section{if $image} has_image{/if}{if $description} has_description{/if}">
		{if $image}
			<div class="cover" href="{url router=PKPApplication::ROUTE_PAGE page="preprints" op="fullSize" type="category" id=$category->getId()}">
				<img src="{url router=PKPApplication::ROUTE_PAGE page="preprints" op="thumbnail" type="category" id=$category->getId()}" alt="null" />
			</div>
		{/if}
	...
	{else}
		...
		{* Pagination *}
		{if $prevPage > 1}
			{capture assign=prevUrl}{url router=PKPApplication::ROUTE_PAGE page="preprints" op="category" path=$category->getPath()|to_array:$prevPage}{/capture}
		{elseif $prevPage === 1}
			{capture assign=prevUrl}{url router=PKPApplication::ROUTE_PAGE page="preprints" op="category" path=$category->getPath()}{/capture}
		{/if}
		{if $nextPage}
			{capture assign=nextUrl}{url router=PKPApplication::ROUTE_PAGE page="preprints" op="category" path=$category->getPath()|to_array:$nextPage}{/capture}
		{/if}
		{include
